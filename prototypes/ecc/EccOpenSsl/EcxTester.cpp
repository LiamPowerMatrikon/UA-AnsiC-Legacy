	...
	OpcUa_ByteString* pCertificate)
{
  ...
  ret = EVP_DigestVerify(ctx, (const unsigned char*)signature.Data, (size_t)signature.Length, (const unsigned char*)pData->Data, (size_t)(pData->Length - keySize * 2));
  if (ret < 1)
  {
    return false;
  }
	...
	OpcUa_ByteString* pSignature)
{
	...
	auto pEcxPrivateKey = d2i_PrivateKey(EVP_PKEY_ED25519, NULL, &pos, pPrivateKey->Length);
  ...
  ret = EVP_DigestSign(ctx, (unsigned char*)pSignature->Data, (unsigned int*)&(pSignature->Length), (const unsigned char*)pData->Data, (int)pData->Length);
  if (ret < 1)
  {
    return false;
  }
...
unsigned int GetSignatureSize(OpcUa_ByteString* pPrivateKey)
{
  ...
  auto keySize = 256;// EC_GROUP_get_degree(EC_KEY_get0_group(pEcPrivateKey));
	...
	OpcUa_ByteString* pPrivateKey)
{
  ...
  pBuffer = pPublicKey->Data;

  pPublicKey->Length = i2d_PUBKEY(pEvpKey, &pBuffer);

  pBuffer = pPrivateKey->Data;

  pPrivateKey->Length = i2d_PrivateKey(pEvpKey, &pBuffer);

  //auto keySize = 256;// EC_GROUP_get_degree(EC_KEY_get0_group(pEcKey));
...
{
	unsigned char* pData = pPrivateKey->Data;
	auto pEcPrivateKey = d2i_PrivateKey(EVP_PKEY_X25519, nullptr, (const unsigned char**)&pData, pPrivateKey->Length);
	if (pEcPrivateKey == nullptr)
	{
		return false;
	}

  auto pEcRemotePublicKey = d2i_PUBKEY(nullptr, (const unsigned char**)&pNonce->Data, pNonce->Length);
  if (pEcPrivateKey == nullptr)
  {
    return false;
  }
  ...
  pSharedSecret->Data = new uint8_t[keylen];
  pSharedSecret->Length = keylen;

  ret = EVP_PKEY_derive(pctx, pSharedSecret->Data, &pSharedSecret->Length);
  if (ret < 1)
  {
    return false;
  }
...
namespace EccOpenSsl {
  ...
  EcxTester::~EcxTester()
	...
	void EcxTester::SetLocalCertificate(String^ certificateFilePath, String^ privateKeyFilePath, String^ password)
	{
	...
	void EcxTester::Decode(String^ requestPath, String^ responsePath, array<unsigned char>^% clientSecret, array<unsigned char>^% serverSecret)
	{
		...
		try
		{
			...
			if (!GenerateKeys(NULL, &m_p->EphemeralPublicKey, &m_p->EphemeralPrivateKey))
			{
	...
	void EcxTester::Encode(String^ certificateFilePath, String^ privateKeyFilePath, String^ password)
	{
		marshal_context context;
		auto pFilePath = context.marshal_as<const char*>(certificateFilePath);
